...
Two pages, linked from a banner at the very top of the homepage (above the hero) plus the footer — neither touches the homepage's own markup, styles, or scripts:

- **`promote.html`** — the leaderboard, a "How it works" explainer, recent activity, and "Your entries" (status on whatever this browser has submitted). Read-only against Firestore; no form here.
- **`promote-submit.html`** — the actual submission form and, after submitting, the payment confirmation panel. Reached via every "List your account" / Boost / Edit & resubmit action.

Splitting these out (an earlier version had the form inline at the bottom of one long scrolling page) keeps the leaderboard page fast and skimmable, and gives the submission flow its own focused page instead of competing for attention with the leaderboard above it. `js/promote-shared.js` holds everything both pages need — constants, `localStorage`/"Your entries" helpers, the deterministic key function, the share-card builder, the theme toggle — loaded before either page's own script so there's no duplication between them.

Ranked purely by bid amount — highest bid at the top, ties broken by whoever got there first. Starting bid is ₹100, and someone already listed can "top up" with a higher bid later to climb, which updates their existing entry instead of creating a duplicate. Three Firestore collections:
...
- **`socialListingRequests`** — the pending queue a submission lands in first. Unreadable from the client (`allow read: if false`) — nobody, including the submitter, can read this collection back, so pending bid amounts and contact details can't be scraped or gamed before review. Writable (bounded field validation) for create/update, and now also **deletable by anyone who knows the id** (`allow delete: if true`) — that's what powers the "🗑 Remove" button in "Your entries," letting a submitter withdraw their own still-pending request. Same trust model as create/update: the id is derived from platform+handle, not secret, and there's no auth here to check an "owner" against — this only ever reaches a *pending* request, never a live listing (`socialListings` stays fully client-write: false) or someone else's approval history, and the admin's manual review remains the real backstop either way. "Still present in this collection" always means "still pending"; approving or rejecting a request deletes it (approving also writes the result into `socialListings`).
...
**All three collections are keyed by a deterministic id** — `platform_handle`, normalized (`js/promote-shared.js`'s `listingKey()`) — instead of an auto-generated one. Resubmitting the same account overwrites its own pending request rather than creating a new one, and the admin tool derives "is this a top-up" by checking whether `socialListings/{that same id}` already exists, straight from Firestore at review time rather than trusting a client-supplied flag. This exists because of a real bug hit in production: the submit button briefly re-enabled itself after a successful submission, and a few impatient re-clicks created four identical pending requests before anyone noticed.

**Submitting is meant to take under a minute.** Pick a platform, type a handle — no `@`, it's shown as a fixed prefix next to the input and stripped automatically if typed anyway — and for the four named platforms (Instagram, YouTube, X, Facebook) the profile link is generated for you (`buildProfileUrl()` in `js/promote-shared.js`, e.g. `https://instagram.com/handle`, or `https://youtube.com/@handle` for YouTube) and shown read-only. A "✏️ Not right? Enter a different link" toggle unlocks manual editing for the rare case the generated link isn't correct; anything typed there is checked against the platform's real domain at submit time (`urlMatchesPlatform()`) so an Instagram entry can't sneak through pointed at some unrelated site — exactly the kind of mistake the auto-generated link exists to avoid in the first place. "Other" has no fixed domain, so its URL field is always manual, with only the baseline `https://` check applied. Handles are still stored with a leading `@` (`data.handle = "@" + handleRaw`) to match how existing entries already render on the leaderboard.

**Payment is entirely manual and off-platform** — this site never touches money. Submitting the form shows a confirmation with the entry's id and a WhatsApp button prefilled with it, plus a UPI ID to pay to; the site owner checks the screenshot (with the entry id in it) against their own UPI payment history and approves by hand. The **local admin tool** (`admin-tool/`, run with `npm run admin-tool`, opens `http://127.0.0.1:5057`) is how that happens — it lists pending requests (each tagged with its entry id, and showing the current listing side-by-side for top-ups) with one-click Approve/Reject. Reject prompts for a short reason (optional, shown to the submitter via "Your entries" below); Approve also clears out any old rejection notice for that same id, in case this is a resubmission after an earlier rejection. It uses the same `serviceAccountKey.json` as `scripts/*.mjs`, binds to `127.0.0.1` only, and **the whole `admin-tool/` directory is gitignored — it never gets pushed, by design.** Approving preserves the listing's original `createdAt` (so a top-up doesn't jump the first-come-first-served tiebreak) while bumping `approvedAt` (so it still surfaces correctly in "recent activity"). *(Reject with a reason only works once you're running the version of the tool that includes it — restart `npm run admin-tool` if it was already running when this shipped, and note that anything rejected before this feature existed has no reason on file and can't be backfilled — there was nothing to recover it from.)*
...
**Outranking an occupied spot needs at least ₹50 more, not just ₹1 more** (`MIN_BOOST_INCREMENT` in `js/promote-shared.js`) — applies whether you're boosting your own listing or a fresh submission targets a rank someone else already holds. `priceForRank()` bakes this in (`holder.bidAmount + MIN_BOOST_INCREMENT`), so the rank calculator, the top-up notice's suggested amount, and the Boost prefill all agree; the submit handler enforces it server-adjacent too (client-side check, same trust level as everything else on this unauthenticated form). Without a floor here, outranking someone would turn into an endless single-rupee back-and-forth instead of bids actually moving the board — a fresh, unoccupied rank still only needs the ₹100 starting minimum.

**"Your entries"** (on `promote.html`) — a section that only appears once `localStorage`'s `listenit_my_listings` has something in it (nothing to do with accounts — there aren't any — just "what did this browser submit"). For each one it shows a live status: `✅ Live — Rank #N, ₹X` if it's in the current `socialListings` snapshot, `❌ Rejected` (with the admin's reason, plus an "✏️ Edit & resubmit" button that jumps to `promote-submit.html` pre-filled with everything originally entered) if it's not live but a matching `socialListingRejections` doc exists, otherwise `⏳ Pending review` (with a "🗑 Remove" button to withdraw it). A rejection only counts if it's newer than this browser's own last submission for that account — the client can't delete the old rejection doc itself (read-only), so instead it compares timestamps: resubmitting via Edit bumps the local `submittedAt` past the old `rejectedAt`, which flips the status straight back to Pending without waiting on the admin. Also what makes the platform+handle-aware duplicate warning possible even before something's approved — typing in a handle that matches a *pending* local submission (not yet a real listing) shows its own notice, distinct from the "you're already live, this is a top-up" one. A different platform for the same person never triggers either warning — the key is per-platform, so listing Instagram and YouTube separately is expected, not a duplicate.

Since the form now lives on its own page, Boost (from the leaderboard or "Your entries") and Edit & resubmit (from "Your entries") hand off through a one-shot `sessionStorage` payload (`goToSubmitWithPrefill()` / `consumePrefill()` in `js/promote-shared.js`) rather than filling in fields directly — `promote-submit.html` reads it once on load, applies it, and clears it.

**Boosting is ownership-gated** — the "⬆ Boost" button on a leaderboard row (and in "Your entries," for a live entry) only renders if `isMine()` (same `listenit_my_listings` check as above) says this browser submitted that entry; everyone else just sees the Share button. Typing a handle into the form that matches someone *else's* live listing shows a red blocking notice and disables submit, and the submit handler itself re-checks the same condition as a backstop. This is enforced entirely client-side against `localStorage` — there's no login on this site to check against, so it's not cryptographic proof (clearing storage or typing a matching handle on another device gets around it), but it stops the obvious case of a random visitor clicking Boost on someone else's card, and the admin still reviews every request by hand before anything actually changes.
